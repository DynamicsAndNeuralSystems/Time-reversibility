...
models=['OU', 'BRW_cont', # Random walks (R)
        'VDP_STOCH', 'VDP', 'Oscillator', # Oscillators (R)
        'NoisySine', # Deterministic with added noise (R)
        'LORENZ_X', 'LORENZ_Y', # Chaotic flow (R)
        'LORENZ_Z', 'LORENZ_SUM', # Chaotic flow (I)
        'LORENZ_STOCH_X', 'LORENZ_STOCH_Y', # Stochastic chaotic flow (R)
        'LORENZ_STOCH_Z', 'LORENZ_STOCH_SUM', # Stochastic chaotic flow (I)
        'ROSSLER_X', 'ROSSLER_Y', 'ROSSLER_Z', 'ROSSLER_SUM', # Chaotic flow (I)
        'Oscillator_STOCH', # Non-stationary (I)
        'MACKEYGLASS17' # Chaotic flow
        ]

model_keywords={'OU': ['reversible'], 'BRW_cont': ['reversible'], # Random walks (R)
        'VDP_STOCH': ['reversible'], 'VDP': ['reversible'], 'Oscillator': ['reversible'], # Oscillators (R)
        'NoisySine': ['reversible'], # Deterministic with added noise (R)
        'LORENZ_X': ['reversible'], 'LORENZ_Y': ['reversible'],'LORENZ_Z': ['irreversible'], 'LORENZ_SUM': ['irreversible'],  # Chaotic flow (I)
        'LORENZ_STOCH_X': ['reversible'], 'LORENZ_STOCH_Y': ['reversible'], 'LORENZ_STOCH_Z': ['irreversible'], 'LORENZ_STOCH_SUM': ['irreversible'], # Stochastic chaotic flow (I)
        'ROSSLER_X': ['irreversible'], 'ROSSLER_Y': ['irreversible'], 'ROSSLER_Z': ['irreversible'], 'ROSSLER_SUM': ['irreversible'], # Chaotic flow (I)
        'Oscillator_STOCH': ['irreversible'], # Non-stationary (I)
        'MACKEYGLASS17': ['irreversible'] # Chaotic flow (I)
        }
...
for model_name in models:
    model_dir = os.path.join(f'../Data_generation/Data_{lenght_ts}_ACFDS', model_name)
